...
class Memory(object):
    ...
    def modular_save(self, obs_path, next_obs_path, action_path, dones_path):
        b = np.asarray(self.buffer, dtype=object)
        observations = np.concatenate(b[:, 0]).reshape(-1, b[0, 0].shape[0])
        next_obs = np.concatenate(b[:, 1]).reshape(-1, b[0, 1].shape[0])
        actions = b[:, 2].reshape(-1, 1)
        dones = b[:, 4]
        np.save(obs_path, observations)
        np.save(next_obs_path, next_obs)
        np.save(action_path, actions)
        np.save(dones_path, dones)
    ...
    for epoch in count():
        ...
        for time_steps in range(200):
            ...
            if memory_replay.size() == REPLAY_MEMORY:
                ...
                memory_replay.modular_save('cartpole_expert_states',
                        'cartpole_expert_next_states',
                        'cartpole_expert_actions',
                        'cartpole_expert_done')
                exit()
            if done:
                break
            state = next_state
