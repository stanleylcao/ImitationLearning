...
class SoftQNetwork(nn.Module):
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            ...
            # print('dist =', dist)
            max = torch.max(dist, dim=1)
            # print('best action =', max[1])
...
def main():
    ...
    saved_policy_file_name = 'sqil-policy-sandbox.para'

    for epoch in count():
        state = env.reset()
        episode_reward = 0
        state = state / env.observation_size_high # normalize
        for time_steps in range(200):
            ...
            next_state = next_state / env.observation_size_high # normalize
            ...
            if online_memory_replay.size() > 1280:
                ...
                expert_batch_state = expert_batch_state / env.observation_size_high # normalize
                expert_batch_state = torch.FloatTensor(expert_batch_state).to(device)
                expert_batch_next_state = expert_batch_next_state / env.observation_size_high # normalize
...
if __name__ == "__main__":
    ...
    # env = expert_demos.env
    # num_actions = env.action_space.n # get number of actions
    # model = SoftQNetwork(num_actions).to(device)
    # model.load_state_dict(torch.load('sqil-policy.para'))
    # model.eval()
    # with torch.no_grad():
    #     for i in range(10):
    #         print(model.choose_action(expert_demos.obs[i].float()))